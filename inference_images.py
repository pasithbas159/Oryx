...
# --- Add: Import dataset creation and splitting ---
...
def eval_model(args, image_files, prompt):
    # Model
    disable_torch_init()
    ...
    image_highres_tensor = image_highres_tensor.to(dtype=torch.bfloat16, device='cuda')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mapped-images-folder", type=str, default=None)
    ...
    for test_sample in test_conversation_dataset:
        test_sample = test_conversation_dataset[0]
        # Extract image file paths and prompt
        user_content = test_sample["messages"][0]["content"]
        image_files = [item["image"] for item in user_content if item["type"] == "image"]
        prompt = ""
